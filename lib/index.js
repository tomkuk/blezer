...
async function listen(pid, queues) {
  while (true) {
    ...
    try {
      ...
      if (args.rootJid) {
        await redis.hincrbyAsync(`blezer:jobs:${args.rootJid}`, 'processed', 1);
        await redis.hincrbyAsync(`blezer:jobs:${args.rootJid}`, 'succeeded', 1);
      }
    } catch (error) {
      ...
      if (job.args.rootJid) {
        ...
        await redis
          .multi()
          .hmset(`blezer:jobs:${rootJid}`, { failedAt: Date.now() })
          .hincrby(`blezer:jobs:${rootJid}`, 'processed', 1);
